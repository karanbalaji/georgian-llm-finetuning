...
### Example Challenge Tracks

Concrete answers to "what would people actually build?" — each framed around **small, efficient fine-tuned models**, which serve on Render's standard infrastructure without GPU dependencies. That constraint isn't a limitation; it *is* the toolkit's core pitch: a small model fine-tuned on your data beats calling a frontier API on cost, latency, and privacy for narrow tasks.

- **Support-ticket triage** — fine-tune a small model to classify and route tickets, served as a Render web service with a demo inbox
- **Domain-specific summarizer** — legal, medical, or financial document summarization behind an API endpoint, benchmarked against a big-API baseline on cost/latency
- **Code-review or log-anomaly assistant** — a fine-tuned model wired into a working demo app, showing the full fine-tune → deploy → integrate loop

Participants can also bring their own use case — the tracks are scaffolding, not walls.
...
**Quality filter, deliberately light-touch:** registration asks for a GitHub/portfolio link (signal without gatekeeping), and the kickoff livestream itself acts as a self-selection gate — people who watch a full technical walkthrough and still register are the ones who'll actually build.

## Timeline (8 weeks, within the 6–10 week window)

| Week | Milestone |
|---|---|
| 1 | **Launch.** Landing page live. Kickoff livestream co-hosted by a Georgian AI Lab engineer and a Render engineer, walking through the fine-tuning toolkit → Render deploy pipeline end-to-end. This session doubles as the technical walkthrough content participants will reference all challenge long. |
| 2–5 | **Build window.** Participants fine-tune and deploy their projects. Office hours + Discord support staffed jointly by Georgian AI Lab and Render engineers. |
| 6 | **Submissions close.** |
| 7 | **Judging.** Georgian AI Lab + Render engineers score against the rubric below. |
| 8 | **Demo Day.** Live showcase (in-person Toronto + streamed), winners announced, recap video and photo gallery published, participants prompted to share their builds (tagging Georgian + Render) for their own visibility. Top performers get fast-tracked into Render's interview pipeline — the direct talent outcome. |
...
- Content output: kickoff-livestream and recap views, participant posts tagging Georgian + Render
...
This is the concrete, working example of "Season 1, Builder Series" from the strategy doc — not a hypothetical program layered on top of it. It's designed to be a repeatable template: the same 8-week shape (kickoff livestream → build window with office hours → judging → Demo Day) could run again with a different portfolio company and toolkit in a later Builder Series episode, without reinventing the format each time.
